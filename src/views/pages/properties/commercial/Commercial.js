...
const Commercial = () => {
  ...
  const { properties } = commercialData;

  const handlePropertyClick = (propertyId) => {
    // Utilisez une route dynamique avec l'ID de la propriété
    navigate(`/ForCommercial/${propertyId}`);
  };
  ...
  return (
    <div className="d-flex flex-column min-vh-100">
      ...
      <main className="bg-black flex-grow-1">
        {/* Bannière */}
        <Row className="m-0">
          <Col className="image-container-commercial image-query p-0">
            <div className="image-title-commercial">
              <h1 className="text-white text-uppercase fw-bold fs-1 nowrap">Nos immeubles commerciaux</h1>
            </div>
          </Col>
        </Row>
