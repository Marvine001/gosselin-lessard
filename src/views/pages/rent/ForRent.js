...
const CATEGORY_CONFIG = {
  'residentiel': {
    ...
    redirectPath: '/ForResidential'
  },
  'commercial': {
    label: 'Commercial', 
    data: commercialData,
    redirectPath: '/ForCommercial'
  },
  'industriel': {
    label: 'Industriel',
    data: industrialData,
    redirectPath: '/ForIndustrial'
  }
};
...
const ForRent = () => {
    ...
    // Formater l'adresse pour l'affichage avec le nouveau format
    ...
    // Fonction pour obtenir l'adresse complète avec code postal
    const getFullAddress = (address) => {
        if (address && typeof address === 'object') {
            const parts = [];
            if (address.adresse) parts.push(address.adresse);
            if (address.ville) parts.push(address.ville);
            if (address.province && address.codePostale) {
                parts.push(`${address.province} ${address.codePostale}`);
            }
            return parts.join(', ');
        }
        return null;
    ...
    // Utiliser useEffect pour charger la propriété
    useEffect(() => {
        setLoading(true);
        
        if (!category || !propertyId || !CATEGORY_CONFIG[category]) {
            // Rediriger si la catégorie ou l'ID de propriété n'existe pas
            navigate('/rent');
            return;
        }
        
        // Rechercher la propriété par ID dans la catégorie correspondante
        const categoryData = CATEGORY_CONFIG[category].data;
        const foundProperty = categoryData.properties.find(p => p.id === parseInt(propertyId));
        
        if (foundProperty) {
            setProperty({
                ...foundProperty,
                categoryType: category // Stocker le type de catégorie pour l'affichage conditionnel
            });
        } else {
                ...
                <Container fluid>
                    <Row className='px-4'>
                        <Col md={12}>
                            <h3 className='text-center text-white uppercase fw-semibold'>
                                {property.title} - {property.location || formattedAddress.city}
                            </h3>
                        </Col>
                    </Row>
                    ...
                    <Row className='mt-5'>
                        ...
                        <Col
                            ...
                            <h3 className='uppercase'>
                                <div>{getFullAddress(property.address)}</div>
                            </h3>
                            ...
                            <div className="property-details mt-4">
                                {/* Détails pour propriété résidentielle */}
                                {property.categoryType === 'residentiel' && (
                                    <>
                                        <div className="detail-item">
                                            <strong>Disponibilité:</strong> {property.availability}
                                        </div>
                                        {property.dateAvailabilty && (
                                            <div className="detail-item">
                                                <strong>Date de disponibilité:</strong> {property.dateAvailabilty}
                                            </div>
                                        )}
                                        {property.typeApartment && (
                                            <div className="detail-item">
                                                <strong>Type d'appartement:</strong> {property.typeApartment}
                                            </div>
                                        )}
                                        {property.price && (
                                            <div className="detail-item">
                                ...
                                {/* Détails pour propriété commerciale */}
                                {property.categoryType === 'commercial' && (
                                    <>
                                        <div className="detail-item">
                                            <strong>Disponibilité:</strong> {property.availability}
                                        </div>
                                        ...
                                        {property.squareFeet > 0 && (
                                            <div className="detail-item">
                                                <strong>Superficie:</strong> {property.squareFeet} pi²
                                            </div>
                                        )}
                                        {property.price && (
                                            <div className="detail-item">
                                ...
                                {/* Détails pour propriété industrielle */}
                                {property.categoryType === 'industriel' && (
                                    <>
                                        <div className="detail-item">
                                            <strong>Disponibilité:</strong> {property.availability}
                                        </div>
                                        {property.dateAvailability && (
                                            <div className="detail-item">
                                                <strong>Date de disponibilité:</strong> {property.dateAvailability}
                                            </div>
                                        )}
                                        {property.squareFeet > 0 && (
                                            <div className="detail-item">
                    ...
                    <Row className='bg-blue text-white text-center uppercase py-4'>
                        <Col xs={12} md={12} lg={12}>
                            <span className='fs-5'>
                                Espace {CATEGORY_CONFIG[property.categoryType]?.label || 'à louer'}
                            </span>
                        </Col>
                    </Row>
